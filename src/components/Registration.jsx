...
export default function Registration() {
  ...
  const [timerState, setTimerState] = useState({ 
    hours: 0, minutes: 0, seconds: 0, status: 'WAITING'
  });

  useEffect(() => {
    const startTime = new Date('2026-06-07T16:00:00+05:30').getTime();
    const endTime = new Date('2026-06-08T16:00:00+05:30').getTime();
    ...
    const interval = setInterval(() => {
      ...
      if (now < startTime) {
        ...
        setTimerState({ hours: 0, minutes: 0, seconds: 0, status: 'EXPIRED' });
      }
    }, 1000);
            ...
            {timerState.status === 'EXPIRED' ? (
              ...
              >
                <motion.div
                  ...
                  className="w-20 h-20 bg-brand-lavender-100 rounded-full flex items-center justify-center mb-6 mx-auto"
                >
                  <svg className="w-10 h-10 text-brand-magenta-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                </motion.div>
                <h3 className="text-2xl md:text-3xl font-bold text-gradient font-display mb-3">
                  Early Bird Slots Closed
                </h3>
                <p className="text-deep-purple-800/60 max-w-sm mx-auto">
                  Early bird slots have now been closed, registrations opening soon.
                </p>
              </motion.div>
            ) : !submitted ? (
              /* ───────── FORM ───────── */
              ...
              >
                ...
                {/* Payment Section */}
                <motion.div
                  className="md:col-span-2 border border-brand-lavender-200 rounded-xl p-6 mt-2 mb-2 relative overflow-hidden"
                  variants={{
                    hidden: { opacity: 0, y: 24 },
                    visible: {
                      opacity: 1,
                      y: 0,
                      transition: { delay: 0.50, duration: 0.5, ease: [0.25, 0.46, 0.45, 0.94] },
                    },
                  }}
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-brand-lavender-100 to-transparent opacity-50 rounded-bl-full pointer-events-none" />
                  <h4 className="text-lg font-bold text-deep-purple-800 font-display mb-4 flex items-center gap-2">
                    <svg className="w-5 h-5 text-brand-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 items-center relative z-10">
                    ...
                    <div className="flex-grow w-full space-y-4">
                        ...
                        <div className="bg-brand-lavender-50/50 px-6 py-5 rounded-[13px] h-full flex flex-col items-center justify-center text-center gap-3">
                          ...
                          <div className="flex flex-col items-center gap-1">
                            <div className="flex items-center gap-2 text-brand-magenta-600 font-bold tracking-wider uppercase text-[11px] sm:text-xs">
                              ...
                              <span>Special 24-Hour Slot</span>
                              <span className="animate-pulse">⚡</span>
                            </div>
                          ...
                          <div className="flex items-center gap-4 bg-white px-5 py-2.5 rounded-xl w-full justify-center border border-brand-lavender-200 shadow-sm mt-2">
                            <svg className={`w-6 h-6 ${timerState.status === 'ACTIVE' ? 'animate-pulse text-brand-magenta-500' : 'text-brand-purple-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            {timerState.status === 'EXPIRED' ? (
                              <span className="font-bold text-brand-magenta-600 tracking-wider uppercase text-sm">Offer Expired</span>
                            ) : (
                              <div className="flex flex-col items-start">
                                <span className="text-[10px] font-bold text-brand-purple-500/70 uppercase tracking-widest mb-0.5">
                                  {timerState.status === 'WAITING' ? 'Starts In' : 'Ends In'}
                                </span>
                                <span className="font-mono text-xl sm:text-2xl font-bold text-brand-purple-600 tracking-widest leading-none">
                                  {String(timerState.hours).padStart(2, '0')}<span className="text-brand-purple-400/50 mx-0.5 sm:mx-1">:</span>{String(timerState.minutes).padStart(2, '0')}<span className="text-brand-purple-400/50 mx-0.5 sm:mx-1">:</span>{String(timerState.seconds).padStart(2, '0')}
                                </span>
                              </div>
                            )}
                          </div>
                      ...
                      <p className="text-sm text-deep-purple-800/70 font-medium leading-relaxed">
                        Scan the QR code to make your registration payment, then upload a screenshot of the confirmation below.
                      </p>
                      ...
                      <div className={`relative flex items-center justify-center w-full mt-2 border-2 border-dashed rounded-lg p-4 transition-colors ${errors.paymentScreenshot ? 'border-red-400 bg-red-50/50' : 'border-brand-lavender-300 hover:border-brand-purple-400 bg-brand-lavender-50/30'}`}>
                        ...
                        <div className="flex flex-col items-center gap-1 text-center pointer-events-none">
                          ...
                          <span className="text-sm font-medium text-deep-purple-800">
                            {fileData.filename ? fileData.filename : 'Upload Payment Screenshot'}
                          </span>
